...
//			String deptName = cstmt.getString(1);
	...
	public List<Employee> getEmpList(){ //전체조회 
		...
		conn = DAO.getConnect();
		String sql = "select employee_id,first_name,last_name,hire_date,email,salary,job_id from employees order by 1 desc";
		Employee emp = null;
	...
	public void updateEmp(Employee emp) {
		conn =DAO.getConnect();
		String sql = "update employee_id set salary = ?,email =? where employee_id =?";
		int rCnt = 0;
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(++rCnt,emp.getSalary());
